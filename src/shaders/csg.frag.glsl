...
void main() {
    ...
    vec3 ray_dir = ray_end - ray_start; // no need to normalize    

    vec3 ray_step = normalize( ray_dir ) * 0.03;

    vec3 ray_pos = ray_start + ray_step;

    const int numSpheres = 2;
    vec4 spheres[numSpheres];
    spheres[0] = vec4( -0.2, 0.0,  -0.8, 0.8 );
    spheres[1] = vec4(  0.2 + 0.3 * cos( 0.0001 * u_frameNum ), 0.0,  -0.8, 0.5 );
    //spheres[2] = vec4( -0.5, 0.3, -0.2, 0.3 );
    //spheres[2] = vec4( -0.3, 0.3, -0.2, 0.3 );


    bool didHit = false;
    for ( float step = 0.0; step < 400.0; step += 1.0, ray_pos += ray_step ) {
        
        if ( didHit ) { break; }
        
        //float dist = 100000000.0; // large number
        float dist = 0.0f;
        
        for ( int sphereIdx = 0; sphereIdx < numSpheres; sphereIdx++ ) {
            vec4 currSphere = spheres[ sphereIdx ];

            //float sphereDist = max( 0.0, distance( ray_pos, currSphere.xyz ) - currSphere.w );
            //float sphereDist = distance( ray_pos, currSphere.xyz ) - currSphere.w;
            float sphereDist = distance( ray_pos, currSphere.xyz );

            dist += currSphere.w / sphereDist;
            //vec3 diff = ray_pos - currSphere.xyz;
            //float squaredDist = dot( diff, diff );
            //float sphereDist = sqrt( squaredDist );
            
            //dist = min( dist, sphereDist );
//            if ( squaredDist < 0.5 ) {
//                dist += (0.25 - squaredDist + squaredDist*squaredDist);
//            }
//            //dist += 1.0 / max( 0.000001, sphereDist );
//            if ( dist <= 0.0 ) { 
//                o_fragColor = vec4(0.7, 0.6, 0.3, 0.9);
//                return;
//            }
        }

        //dist /= float( numSpheres );
        if ( dist >= 5.0 ) { 
            //o_fragColor = vec4(0.7, 0.6, 0.3, 0.9);
            //return;
            didHit = true;
    ...
    if (didHit) {
        o_fragColor = vec4(0.9, 0.6, 0.3, 0.5);
    } else {
        //discard;
        o_fragColor = vec4(0.1, 0.9, 0.3, 0.5);
    }
...
void main_DVR() {
    ...
    for ( float currStep = 0.0; currStep < lenInVolume; currStep += RAY_STEP_SIZE, curr_sample_pos_noRand += vol_step_ray ) {
        ...
        //color.rgb = ( color.rgb - u_minMaxScaleVal.x ) * u_minMaxScaleVal.z; // map volume-tex values to 0-1 after the loop
        //color.a /= ( lenInVolume / RAY_STEP_SIZE );
        //color.rgb *= color.a;
